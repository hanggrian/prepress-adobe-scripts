...
// https://stackoverflow.com/questions/60509557/regex-remove-leading-zeros-but-keep-single-zero
var REPLACE_LEADING_ZERO = /^(?:0+(?=[1-9])|0+(?=0$))/mg;

/** Restricts input to be digits-only (digits are non-negative and non-decimal number). */
EditText.prototype.validateDigits =
    function() {
      Internals.registerValidator(
          this,
          MATCH_DIGITS,
          function(_, newValue) {
            return Internals.removeRegexes(
                newValue,
...
EditText.prototype.validateUnits =
    function() {
      Internals.registerValidator(
          this,
          MATCH_UNITS,
          function(oldValue, newValue) {
            ...
            if (exec !== null) { // new unit type found, use it
              var newUnitValue =
                  Internals.removeRegexes(
                      newRegexes.substring(0, exec.index),
 ...
 */
EditText.prototype.addChangeListener =
    function(listener) {
 ...
 * @return {!EditText}
 */
Panel.prototype.editText =
    function(size, text, properties) {
      return Internals.addEditText(this, size, text, properties);
    };

Internals.addEditText =
