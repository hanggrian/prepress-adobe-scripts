...
function AboutPanel(parent, clientDate) {
  ...
  self.preferencesTab
  self.preferencesThemeList
  self.preferencesLanguageList
  self.preferencesActivateControl
  self.preferencesClearButton
  self.updatesTab, self.updatesStatusText, self.updatesDownloadButton
  self.licensingTab

  self.preferencesTab =
      self.vtab(R.string.preferences, function(tab) {
        ...
        tab.hgroup(function(group) {
          ...
          self.preferencesThemeList =
              group.dropDownList(undefined, Theme.list()).apply(function(it) {
          ...
          self.preferencesLanguageList =
              group.dropDownList(undefined, Language.list()).apply(function(it) {
        ...
        tab.hgroup(function(group) {
          ...
          self.preferencesActivateControl =
              group.checkBox(undefined, R.string.activate_control_on_start).apply(function(it) {
                it.value = preferences2.getBoolean('activate_control_on_start')
              })
        })
        self.preferencesClearButton =
            tab.button(undefined, R.string.clear_preferences).apply(function(it) {
              it.maximumSize.height = 20
            })
      })
  self.updatesTab =
      self.vtab(R.string.updates, function(tab) {
        tab.preferredSize = SIZE_ABOUT_TAB
        tab.margins = MARGINS_ABOUT_TAB
        tab.alignChildren = 'left'
        ...
        tab.hgroup(function(group) {
          group.button(undefined, R.string.check_updates).apply(function(it) {
            ...
            it.addClickListener(function() {
              ...
              if (!updateFile.exists) {
                ...
                var serverDate =
                    parseDate(
                        updateFile.readText().substringAfter('"date": "')
                            .substringBefore('"')
                            .substring(0, 10),
                    )
                updateFile.remove()
                if (serverDate > clientDate) {
                  self.updatesStatusText.text =
                      getString(
                          R.string.message_aboutscripts_updates_available,
          ...
          self.updatesDownloadButton =
              group.button(undefined, R.string.download).apply(function(it) {
                it.maximumSize.height = 20
                it.enabled = false
                it.addClickListener(function() {
  ...
  self.licensingTab =
      self.vtab(R.string.licensing, function(tab) {
        tab.editText(
            SIZE_ABOUT_TAB,
            Scripts.getResource('LICENSE').readText(),
