  ...
  self.preferencesTab = self.vtab(R.string.preferences, function(tab) {
    ...
    tab.hgroup(function(group) {
      ...
      self.preferencesThemeList = group.dropDownList(undefined, Theme.list()).also(function(it) {
        it.selection = configs.getBoolean("theme_dark") ? 0 : 1
      })
    })
    tab.hgroup(function(group) {
      ...
      self.preferencesLanguageList = group.dropDownList(undefined, Language.list()).also(function(it) {
        var currentCode = configs.getString("language_code", Language.EN.code)
        it.selectText(Language.valueOfCode(currentCode).name)
      })
    })
