...
var Rounding =
    new Enum(
        {
          NONE: {
            text: R.string.none, round: function(it) {
              return it
            },
          },
...
dialog.vgroup(function(main) {
  ...
  main.hgroup(function(group) {
    ...
    dimensionWidthRadio =
        group.radioButton(undefined, R.string.width).apply(function(it) {
          it.addClickListener(changeDimensionText)
          it.select()
        })
    dimensionHeightRadio =
        group.radioButton(undefined, R.string.height).apply(function(it) {
          it.addClickListener(changeDimensionText)
        })
  })
  main.hgroup(function(group) {
    ...
    dimensionSizeEdit =
        group.editText(SIZE_INPUT, formatUnits(item.width, unitType, 2)).apply(function(it) {
          it.validateUnits()
          it.activate()
        })
  })
  main.hgroup(function(group) {
    ...
    roundingList =
        group.dropDownList(SIZE_INPUT, Rounding.list()).apply(function(it) {
          it.selection = 0
        })
  })
})
