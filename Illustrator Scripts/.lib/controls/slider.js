 ...
 */
function SliderGroup(parent, editBounds, current, start, end, scale) {
  var self = this
  ...
  this.main = parent.hgroup(function(group) {
    self.edit = group.editText([40, editBounds[1]], current * scale).also(function(it) {
      it.validateDigits()
      it.onChanging = function() {
        self.slider.value = new Number(it.text) / scale
      }
    })
    self.slider = group.slider([editBounds[0] - 40 - 10, editBounds[1]], current, start, end).also(function(it) {
      it.onChanging = function() {
