...
function RangingGroup(parent, inputSize) {
  ...
  self.startEdit = self.editText(inputSize, '1').apply(function(it) {
    it.validateDigits()
  ...
  /** @return {number} */
  self.getStart = function() { return parseInt(self.startEdit.text) }

  /** @return {number} */
  self.getEnd = function() { return parseInt(self.endEdit.text) }

  /** @return {Range} */
  self.get = function() { return self.getStart().until(self.getEnd()) }
