var Ordering =
    new Enum({
      ...
      layerValues: function() {
        return [Ordering.LAYER_DEFAULT, Ordering.LAYER_REVERSED]
      },
      nameValues: function() {
        return [Ordering.NAME_ASCENDING, Ordering.NAME_DESCENDING]
      },
      ...
      layerList: function() {
        return Collections.map(Ordering.layerValues(), function(it) {
          return [it.text, it.image]
        })
      },
      nameList: function() {
        return Collections.map(Ordering.nameValues(), function(it) {
          return [it.text, it.image]
        })
      },
      positionList: function() {
        return Collections.map(Ordering.positionValues(), function(it) {
          return [it.text, it.image]
        })
      },
    })
...
function OrderingList(parent, ordersCollection) {
  ...
  var self =
      parent.dropDownList(undefined, orders).apply(function(it) {
  ...
  self.getComparator =
      function() {
        return Ordering.find(self.selection).get
      }
