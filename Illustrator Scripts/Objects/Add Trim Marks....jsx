  ...
  main.hgroup(function(rootPane) {
    ...
    rootPane.vpanel(R.string.trim_marks, function(panel) {
      ...
      panel.hgroup(function(group) {
        ...
        offsetEdit =
            group.editText(SIZE_INPUT, prefs.getString('offset', '2.5 mm')).apply(function(it) {
              it.validateUnits()
              it.activate()
            })
      })
      panel.hgroup(function(group) {
        ...
        lengthEdit =
            group.editText(SIZE_INPUT, prefs.getString('length', '2.5 mm')).apply(VALIDATE_UNITS)
      })
      panel.hgroup(function(group) {
        group.helpTips = R.string.tip_addtrimmarks_weight
        group.staticText(undefined, R.string.weight).apply(HEADING)
        weightEdit =
            group.editText(SIZE_INPUT, prefs.getString('weight', '0.3 pt'))
    ...
    rootPane.vpanel(R.string.locations, function(panel) {
      panel.hgroup(function(group) {
        ...
        topLeftMarkCheck =
            group.checkBox(SIZE_CHECK).apply(function(it) {
              it.select()
