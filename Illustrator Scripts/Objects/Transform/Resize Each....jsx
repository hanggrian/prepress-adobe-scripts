...
dialog.vgroup(function(main) {
  main.vgroup(function(topPane) {
    ...
    topPane.hgroup(function(group) {
      ...
      widthEdit =
          group.editText(SIZE_INPUT, formatUnits(prefill.width, unitType, 2)).apply(function(it) {
            it.validateUnits()
            it.activate()
          })
      widthCheck =
          group.checkBox(SIZE_INPUT_CHECK).apply(function(it) {
            it.select()
    ...
    topPane.hgroup(function(group) {
      ...
      heightEdit =
          group.editText(SIZE_INPUT, formatUnits(prefill.height, unitType, 2)).apply(VALIDATE_UNITS)
      heightCheck =
          group.checkBox(SIZE_INPUT_CHECK).apply(function(it) {
            it.select()
            it.addClickListener(function() {
