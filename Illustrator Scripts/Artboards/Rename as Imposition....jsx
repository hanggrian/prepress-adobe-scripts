...
dialog.vgroup(function(main) {
  ...
  main.hgroup(function(group) {
    ...
    impositionList =
        group.dropDownList(SIZE_INPUT, Pager.list()).apply(function(it) {
...
dialog.setDefaultButton(undefined, function() {
  ...
  if (pager === Pager.TWO_UP) {
    ...
    pager =
        pager.get(document, start, nupGroup.isFolding(), nupGroup.isDuplex(), nupGroup.isStack())
  } else {
    pager = pager.get(document, start, document.artboards.length + 1)
  }
  Collections.forEach(document.artboards, function(it) {
    it.name = pager.next()
  })
  return false
})
